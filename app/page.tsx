...
const pageStyles = `
  ...
  .hp {
    ...
    --rose: #E91E8C; --bg: #F4F1EC; --card-bg: #fff;
  ...
  .hp-eyebrow {
    ...
    background: rgba(0,204,255,0.12); border: 1px solid rgba(0,204,255,0.3);
  ...
  .hp-subtitle {
    color: rgba(255,255,255,0.6); font-size: 1.1rem; font-weight: 300;
    max-width: 560px; margin: 0 auto 2.5rem; line-height: 1.75;
  }
  .hp-search-wrap { max-width: 600px; margin: 0 auto 2.5rem; }
  .hp-cats {
    display: flex; flex-wrap: wrap;
    justify-content: center; gap: 10px; margin-bottom: 3.5rem;
  }
  .hp-cat-pill {
    display: inline-flex; align-items: center; gap: 8px;
    padding: 10px 20px; border-radius: 100px;
    background: rgba(255,255,255,0.07); border: 1px solid rgba(255,255,255,0.12);
    color: rgba(255,255,255,0.85); font-size: 0.87rem; font-weight: 500;
  ...
  .hp-cta-solid {
    ...
    box-shadow: 0 8px 30px rgba(247,176,0,0.3); margin-bottom: 3rem;
  }
  .hp-cta-solid:hover { transform: translateY(-3px) scale(1.02); box-shadow: 0 14px 40px rgba(247,176,0,0.45); }
  .hp-stats {
  ...
  /* ── PILLARS ───────────────────────────────── */
  .hp-pillars { background: #fff; padding: 4.5rem 1.5rem; border-bottom: 1px solid #eee; }
  .hp-pillars-grid {
  ...
  .hp-section-sub { font-size: 0.9rem; color: #8a9aaa; margin-top: 6px; font-weight: 300; }
  .hp-view-all {
    display: inline-flex; align-items: center; gap: 6px;
    font-size: 0.85rem; font-weight: 600;
    color: var(--cyan); text-decoration: none; transition: gap 0.2s; white-space: nowrap;
  }
  ...
  .feat-overlay {
    position: absolute; inset: 0;
    background: linear-gradient(to top, rgba(0,34,64,0.7) 0%, transparent 55%);
  ...
  .feat-cat-tag {
    ...
    background: rgba(0,34,64,0.65); backdrop-filter: blur(8px);
  ...
  /* row: name + stars — name clips at 2 lines, stars never wrap */
  .feat-head {
  ...
  .feat-rating-num { font-size: 0.68rem; color: rgba(255,255,255,0.4); white-space: nowrap; }
  ...
  .feat-addr-icon { flex-shrink: 0; }

  /* description: exactly 2 lines */
  .feat-desc {
    font-size: 0.855rem; color: rgba(255,255,255,0.6);
    line-height: 1.65; margin-bottom: 1.1rem;
    flex: 1; /* grows to push footer down */
    display: -webkit-box; -webkit-line-clamp: 2; -webkit-box-orient: vertical; overflow: hidden;
  }

  /* footer: always flush to bottom, hours truncated */
  .feat-footer {
  ...
  .feat-hours {
    ...
    overflow: hidden; white-space: nowrap; text-overflow: ellipsis;
    min-width: 0; flex: 1; /* allows truncation inside flex */
  }
  .feat-hours-icon { flex-shrink: 0; }
  .feat-cta {
    display: inline-flex; align-items: center; gap: 5px;
    font-size: 0.8rem; font-weight: 700; color: var(--cyan);
    white-space: nowrap; flex-shrink: 0; transition: gap 0.2s;
  }
  .feat-card:hover .feat-cta { gap: 9px; }

  /* ── SKELETONS ─────────────────────────────── */
  .sk-card { background: var(--navy); border-radius: 22px; overflow: hidden; }
  .sk-img {
    height: 190px;
    background: linear-gradient(90deg, rgba(255,255,255,0.05) 0%, rgba(255,255,255,0.1) 50%, rgba(255,255,255,0.05) 100%);
    background-size: 200% 100%; animation: sk-shine 1.4s infinite;
  }
  .sk-line {
    border-radius: 6px;
    background: linear-gradient(90deg, rgba(255,255,255,0.05) 0%, rgba(255,255,255,0.1) 50%, rgba(255,255,255,0.05) 100%);
      ...
      <div className="hp">
        ...
        <main>
          ...
          <section className="hp-hero">
            ...
            <div className="hp-hero-orb3" /><div className="hp-hero-grid" />
            ...
            <div style={{ position: "relative", zIndex: 1, textAlign: "center" }}>
              ...
              <motion.div variants={fadeUp} initial="hidden" animate="show" custom={5}>
                <Link href="/empreendimento-solidario" className="hp-cta-solid">
                  <Heart size={17} fill="currentColor" />Economia Solidária<ArrowRight size={15} />
                </Link>
              </motion.div>
              ...
              {loading ? (
                Array.from({ length: 3 }).map((_, i) => (
                  <div className="sk-card" key={i}>
                    ...
                    <div style={{ padding: "1.3rem 1.4rem", display: "flex", flexDirection: "column", gap: 10 }}>
                      ...
                      <div className="sk-line" style={{ height: 13, width: "80%" }} />
                    </div>
                  </div>
                ))
                ...
                featuredBusinesses.map((business, idx) => (
                  <motion.div
                    key={business.id}
                    variants={fadeUp} initial="hidden" animate="show" custom={idx}
                    style={{ height: "100%" }}
                  >
                    <Link href={`/estabelecimento/${business.id}`} className="feat-card">
                      ...
                      <div className="feat-img-wrap">
                        ...
                        {/* category tag */}
                        {business.category && (
                          <div className="feat-cat-tag">{business.category}</div>
                        )}
                      ...
                      <div className="feat-body">
                        ...
                        <div className="feat-addr">
                          <MapPin size={12} color="#00CCFF" className="feat-addr-icon" style={{ flexShrink: 0 }} />
                          {business.address}
                        </div>
                        ...
                        {/* footer — hours truncated + CTA */}
                        <div className="feat-footer">
                          <div className="feat-hours">
                            <Clock size={13} color="#00CCFF" className="feat-hours-icon" style={{ flexShrink: 0 }} />
